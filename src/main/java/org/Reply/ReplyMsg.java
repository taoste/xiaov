...
public class ReplyMsg {
	private static String[] defaultErrorReply = new String[]{"出错了喵","爆炸了喵！","爆裂！爆裂！爆裂！","艾-啦-哎-辣-E-R-R-O-R","警告！生命值不足！",
	...
	public static String reply(String content,String username){
		...
		try {
			...
			if(content.equals("运势")||content.equals("今日运势")){
				content="``"+content;
			}
			if(content.startsWith("`")||content.startsWith("·")||content.startsWith("ˋ")){
				...
				if(ret.equals("")){
	                ...
	                if (content.contains("百百")||content.contains("baka机器人")) {
	                	ret = TuringBot.chat(username, content.replaceAll("@", "").replaceAll("baka机器人","百百"));
	                }
		} catch (Exception e) {
